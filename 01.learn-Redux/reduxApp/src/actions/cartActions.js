...
export const getCart = () => async dispatch => {
    try {
        ...
        dispatch({ type: "GET_CART_REJECTED", message: 'ERROR when getting the cart from session' });
    }
}
...
export const updateCart = (_id, unit, cart) => async dispatch => {
    ...
    let cartUpdate = [...currentBookToUpdate.slice(0, indexToUpdate),
        ...
        try {
            ...
            dispatch({ type: "UPDATE_CART_REJECTED", message: 'ERROR when adding to the cart' });
            console.log(err);
        }
}
...
export const deleteCartItem = (cart) => async dispatch => {
    try {
        ...
        dispatch({ type: "DELETE_CART_ITEM_REJECTED", message: 'ERROR when deleting an item from the cart' });
    }
}